...
> 
> ✨ **Novidades Recentes (Ciclo 37):** Tela inicial do App do Cliente idêntica à referência visual oficial (logotipo 'D', tipografia 'DNA AUTO', slogan 'Seu veículo sempre protegido.', supercarro escuro frontal com faróis azuis em LED e botões oficiais 'Entrar' e 'Cadastrar'). App do cliente em **fullscreen travado e bloqueado** (100% da tela, sem molduras ou rolagem externa da janela). Exibição completa de **todos os dados do carro na fase de busca por placa** (placas Mercosul e antiga, ano fab/mod, cor, motorização, cilindradas, transmissão, combustível, carroceria, segmento, lotação, eixos, peso/tração, chassi e renavam protegidos, situação Detran e cotação FIPE com score) e consulta contínua na aba Veículo. **Relatório Completo de Manutenções & Venda** ultra-completo com emissão em 1 clique via ID ou placa, 40+ seções, peças, notas fiscais, inspeção 360°, QR Code e impressão/PDF. 45 testes automatizados aprovados com 100% de sucesso.
